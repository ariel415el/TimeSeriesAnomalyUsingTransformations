...
def test(args, model, device, test_loader, all_matrices, debug_dir=None):
    model.eval()
    ...
    with torch.no_grad():
        for i, (serie, label) in enumerate(test_loader):
            ...
            permed_series = []
            for j,mat in enumerate(all_matrices):
                mult = np.matmul(serie.detach().cpu().numpy(), mat).astype(np.float32)
                permed_series += [torch.tensor(mult).squeeze(0).cpu()]
            ...
            target = torch.tensor(range(len(all_matrices))).view(len(all_matrices),1).cuda()
            
            correct = pred.eq(target.view_as(pred)).sum().item()

            score = correct / float(len(all_matrices))
...
if __name__ == '__main__':
    ...
    model = model_calss(NUM_MTRICES).to(device)

    matrices_file = os.path.join(args.train_dir, "mats.npy")
    if args.trained_model != "":
        ...
        matrices = [np.random.random((128,128)) for i in range(NUM_MTRICES)]
        os.makedirs(args.train_dir, exist_ok=True)  
        with open(matrices_file,"wb") as f:
            np.save(f ,np.stack(matrices,axis=0))

    val_dataset = data_loader.balls_dataset(frame_w=128, frame_h=128, video_length=64, num_videos=100, matrices=matrices)
    if args.test:
        ...
        val_dataset.test()
        val_dataset.set_matrices(matrices)
        test_loader = torch.utils.data.DataLoader(val_dataset, batch_size=1, shuffle=True, **kwargs)

        test(args, model, device, test_loader, val_dataset.get_matrices())
        exit()
    else:
        train_dataset = data_loader.balls_dataset(frame_w=128, frame_h=128, video_length=64, num_videos=1000, matrices=matrices)
